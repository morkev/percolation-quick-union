...
public class Percolation {
	...
	public Percolation(int N){
		if(N < 0) {
			throw new IllegalArgumentException("N must be greater than 0."); //Throw exception if N is <= 0
		}
		...
		for (int i = 0; i < n; i++) { //iterating only n times, one side of the grid
			 ...
			 * (e.g. if N = 5, 25-(sites #0 through #4)-1 will give us the bottom numbers of the grid.)*/
	...
	public void open(int i, int j){
	    ...
	    /* 
	     * Check site to the left. If that site is open, create union between current site
	     * and site to the left (p and q).
	    ...
	    // number of open cells increases by 1 after condition
	    numberOpen++;
	}
	...
	public boolean isOpen(int i, int j){
		outOfBounds(i, j);	// Checks if site is out of bounds.
